...
clf
close all
n = 220;% total number of cells
rc=300; % radius of circle in pixels
cellsz_pxl =35;% cell size in pixels , agrees with imagestaken on 20X SDconfocal
%synthetic_trackID = 2;% cell ID to watch
nbrh_pxl_sz = 70; % local neighborhood size (! 2 cell sizes)
scale_rnd = 8; % multiply rand number btw 0 and 1  (randomly generated cell displacements)
% the random displacement is  scale_rnd*randn (check the distribution of
% generated numbers) with various values of scale_rnd
% can be adjusted bdepending on cell density
% Experimentally: the cells move on average ~ 5 pixels in x or y.
celltyperatio_equal = 1;% if not 1, then the ratio is ~ 0.6 to 0.4

J = 1;
[cells1,cells2]=seed_cells_rand(rc,n,cellsz_pxl,celltyperatio_equal);% generates a set of cells of two tyoes, intexed with ID and not overlapping
 fig = 1;
...
MCstep = 40;
cellstomove = 10;
dH1 = zeros(size(cells1,1),1);% size as the number of cells
dH2 = zeros(size(cells1,1),1);% size as the number of cells
MC_iter_data = struct;
...
if no_neighbors == 1 % always accept the move if cell had no neighbors
    ...
    if synthetic_trackID==1
         disp(cells1_tmp(synthetic_trackID,:))
         end
else
...
% update the interaction energy for this cell
cells1_tmp(synthetic_trackID,4)=H2(synthetic_trackID);
...
% update the interaction energy for this cell
cells2_tmp(synthetic_trackID,4)=H2(synthetic_trackID);
if dH2(synthetic_trackID)<=0 % if the local cell interaction energy decreased
     cells2(synthetic_trackID,:)=cells2_tmp(synthetic_trackID,:); % update the new cell coordinate 
else 
...
disp(['finished iteration ' num2str(MCiter)]);
end
%% plot cells1
close all
N = 4;
...
figure(7), plot(maketrack(1,1),maketrack(1,2),'rp','MarkerFaceColor','b','Markersize',11);hold on
figure(8), plot(maketrack(end,1),maketrack(end,2),'rp','MarkerFaceColor','b','Markersize',11);hold on
...
figure(7), plot(maketrack(1,1),maketrack(1,2),'rp','MarkerFaceColor','r','Markersize',11);hold on
figure(8), plot(maketrack(end,1),maketrack(end,2),'rp','MarkerFaceColor','r','Markersize',11);hold on
